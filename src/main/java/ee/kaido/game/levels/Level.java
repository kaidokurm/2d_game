package ee.kaido.game.levels;

import ee.kaido.game.entities.Crabby;
import ee.kaido.game.entities.Pinkstar;
import ee.kaido.game.entities.Shark;
import ee.kaido.game.objects.*;

import java.awt.*;
import java.awt.image.BufferedImage;
import java.util.ArrayList;

import static ee.kaido.game.main.Game.TILES_IN_WIDTH;
import static ee.kaido.game.main.Game.TILES_SIZE;
import static ee.kaido.game.utilz.Constants.EnemyConstants.*;
import static ee.kaido.game.utilz.Constants.ObjectConstants.*;
...
public class Level {
    ...
    private void loadLevelData(int redValue, int x, int y) {
        ...
        switch (redValue) {
            case 0, 1, 2, 3, 30, 31, 33, 34, 35, 36, 37, 38, 39 ->
                    grass.add(new Grass((int) (x * TILES_SIZE), (int) (y * TILES_SIZE) - TILES_SIZE, getRndGrassType(x)));
        }
    }
    ...
    private void loadObjects(int blueValue, int x, int y) {
        switch (blueValue) {
            ...
            case BOX, BARREL -> containers.add(new GameContainer(x * TILES_SIZE, y * TILES_SIZE, blueValue));
            ...
            case TREE_ONE, TREE_TWO, TREE_THREE ->
                    trees.add(new BackgroundTree(x * TILES_SIZE, y * TILES_SIZE, blueValue));
        }
    }
